...
# ╔═╡ 6e7b75fc-f508-49d1-a3d3-c321aa5ac916
using PyCall, UnROOT, FHist, StatsPlots, DataFrames

# ╔═╡ 2583e8a9-e0ba-444d-aadf-7b2c7e6d3cb3
using StatsBase

# ╔═╡ c8bb1052-be7a-11ed-2cab-19071d9afb79
md"""
The purpose of this notebook is to use `PyUnfold` library from `https://jrbourbeau.github.io/pyunfold/index.html` in order to apply unfolding on the decay vs escaped angular distribution. 
"""

# ╔═╡ e5d4c870-166e-4c87-b000-eaa2a2dc1ad8
begin 
	gr() 
	default(fmt = :jpg)
	theme(
...
# ╔═╡ 4cf0549e-7824-48c9-8243-6635601da7a7
unfolding = pyimport("pyunfold");

# ╔═╡ 4b871bb8-838c-460c-bcb0-a7893df6e330
callbacks = pyimport("pyunfold.callbacks");

# ╔═╡ 74488da6-e3ab-4fba-8275-43e02c47d9c6
f = ROOTFile(
	"/home/shoram/Work/PhD_Thesis/Job17/Data/Spectrum_G0_G2_G22_G4_kappa_m06583_xi31_06_xi51_01397_1e8E.root"
)

# ╔═╡ 4d0893a7-81a0-4705-ba37-2aa377aee992
tree = LazyTree(f, "tree", ["theta","phi"])[1:100_000,:] 

# ╔═╡ 36411163-b234-421b-989c-b27d96464a61
bins = 0:15:180

# ╔═╡ 8b784544-7061-4248-8507-afa9bad28018
theta_hist = Hist1D(tree.theta, bins)

# ╔═╡ 84bc8145-6937-49f5-a898-2153a3f95fb0
phi_hist = Hist1D(tree.phi, bins)

# ╔═╡ 24d1fafb-624c-4539-9a0b-c6970046a523
data_true = bincounts(theta_hist);

# ╔═╡ 47388e10-d4ac-4913-b160-dfe07cdc2bc2
data_obs = bincounts(phi_hist);

# ╔═╡ 76953290-1579-4c3d-8b94-a465d9a59e82
data_obs_err = sqrt.(data_obs);

# ╔═╡ f5fa1e59-fbee-468a-8991-5fc5cc15bcf2
data_type = Float64#typeof(data_obs[1]);

# ╔═╡ 1273c34f-ba81-4095-976d-103587bf0af5
n = length(data_obs)

# ╔═╡ 321bf479-1428-4124-bc1e-8435171cd0ef
efficiencies = ones(eltype(data_type),n);

# ╔═╡ 7bb3ca7b-70dc-4c9e-a892-996269269b76
efficiencies_err = 0.1 .* efficiencies;

# ╔═╡ b06dca7b-ef7d-4e4b-83d0-b50f8aea8969
response_hist = Hist2D(
	(tree.phi, tree.theta),
	(bins, bins)
)

# ╔═╡ 0212e44e-aa1f-4102-86ae-962afc0b938e
response = bincounts(response_hist);

# ╔═╡ 531a05ea-378c-4dbc-a861-c923e511e914
response_err = sqrt.(response) ;

# ╔═╡ 4f3150e9-41d9-4970-b0ad-fd76953bd822
column_sums = [sum(response[:,i]) for i in 1:size(response)[2] ];

# ╔═╡ 3bc059c3-ca04-4a1a-8699-17f761000b5c
normalization_factor = efficiencies ./ column_sums;

# ╔═╡ 6b153fbb-bfb5-4899-895b-abf8dd5ffecd
response_normed = response .* normalization_factor;

# ╔═╡ 3289a6e9-4c60-4c6a-879d-77efe84ddd3b
response_err_normed = response_err .* normalization_factor;

# ╔═╡ 76e60038-e7b9-4bed-bf42-e721b64bead9
response_hist.hist.weights .* normalization_factor;

# ╔═╡ 4d090be3-0a4d-461a-8b7c-a8a7a0cd0e00
unfolded_results = unfolding.iterative_unfold(
	data=data_obs,
	data_err=data_obs_err,
	response=response_normed,
	response_err=response_err_normed,
	efficiencies=efficiencies,
	efficiencies_err=efficiencies_err,
	callbacks=[callbacks.Logger()]
)

# ╔═╡ 9dae86b5-1b21-46f9-bd24-468518978410
stephist(tree.theta, nbins = Int(180/15))

# ╔═╡ e7bea7bc-a51f-4b30-9b1a-78f7d5811ed7
scatter!(
	bincenters(theta_hist),
	unfolded_results["unfolded"]
)

# ╔═╡ 09502d71-ec60-4e7f-95fc-4f9f6d0db8cc
sh= StatsBase.fit(Histogram, tree.theta, nbins = 12)

# ╔═╡ 30395515-28d9-49ac-abb6-02698da427b2
midpoints(sh.edges[1])

# ╔═╡ 6b77318e-4c3a-44dc-9b36-956fc42252f9
plot(sh, fillcolor= false)

# ╔═╡ a3a99425-5ee8-4479-ae71-84073f6aae07
scatter!(
	midpoints(sh.edges[1]),
	unfolded_results["unfolded"],
...
# ╔═╡ 24c7bf6e-d598-416a-ba17-f21e5aeff67f
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "df45ad9e1ff39dd91237138d43192c7301d05f64"
...
git-tree-sha1 = "fbb8e857313552d2a25ca04a9d20b1f899a498cb"
...
git-tree-sha1 = "a7086e1950d6aae93f4dfcd56b48ed17aa66bc22"
